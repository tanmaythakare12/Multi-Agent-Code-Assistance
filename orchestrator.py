...
# Runtime-class errors the debugger is also allowed to attempt a fix for.
# Unlike syntax errors, these can stem from logic mistakes, so the
# AST-equivalence gate in `debug()` is what keeps this safe: a fix is only
# accepted if it does not change the program's structure. In practice this
# means the debugger can still only correct superficial issues (e.g. an
# off-by-one in a literal, a wrong comparison operator) that don't alter the
# tree shape - true logic rewrites will still fail the AST check and be
# rejected, same as before.
RUNTIME_ERROR_MARKERS = [
    "typeerror", "valueerror", "indexerror", "keyerror", "attributeerror",
    "zerodivisionerror", "nameerror", "assertionerror", "recursionerror",
]

REPAIRABLE_ERROR_MARKERS = SYNTAX_ERROR_MARKERS + RUNTIME_ERROR_MARKERS
...
SANDBOX_MEMORY_BYTES = 256 * 1024 * 1024  # 256 MB address space cap


def _sandbox_limits():
    """
    Applied in the child process (via preexec_fn) before exec'ing python3.
    Enforces:
      - CPU time limit (kills runaway/infinite-loop code even without the
        wall-clock timeout below, since a tight CPU-bound loop hits this
        first)
      - Address space / memory limit (kills memory-exhaustion attempts)
      - No core dumps
    NOT enforced here (documented limitation, not silently ignored):
      - Network access is not blocked at the OS level. This relies on the
        sandboxed code not making outbound calls; it does not prevent them.
        A production version would need a network namespace or firewall
        rule, not just a Python-level control.
      - Filesystem access outside the working directory is not blocked by
        a kernel-level mechanism (no chroot/namespace) - it is restricted
        only by environment/cwd setup below, which a determined adversarial
        script could still escape (e.g. via absolute paths). True isolation
        would require a container (Docker/gVisor/Firecracker) or seccomp.
    """
...
def run_candidate(code, test_code):
    """
    Execute candidate code + test code in a resource-limited subprocess.

    Isolation provided:
      - Separate OS process (crash/exception in candidate doesn't affect
        the orchestrator)
      - Wall-clock timeout (kills hangs/infinite loops that don't trip the
        CPU limit, e.g. blocking I/O waits)
      - CPU time limit + memory limit (via _sandbox_limits, see caveats
        in that docstring)
      - Runs from a dedicated temp directory with a stripped-down
        environment (no inherited secrets/env vars), rather than the
        orchestrator's own working directory/environment
    Isolation NOT provided (see _sandbox_limits docstring): kernel-level
    filesystem/network confinement. This is process + resource isolation,
    not full container-grade sandboxing.
    """
    sandbox_dir = tempfile.mkdtemp(prefix="codeweave_sandbox_")
    tmp_path = os.path.join(sandbox_dir, "candidate.py")
    with open(tmp_path, "w") as f:
        f.write(code + "\n" + test_code)

    minimal_env = {"PATH": "/usr/bin:/bin", "HOME": sandbox_dir}
    ...
    try:
        ...
        return "FAIL: execution timed out (possible infinite loop)"
    finally:
...
class CodingPipeline:
    ...
    def debug(self, buggy_code, error_class, test_code):
        """
        Two repair modes, with different safety gates:

        - 'syntax': the fix must compile AND its AST must be IDENTICAL to
          the original (modulo line/col metadata). This guarantees the
          debugger cannot alter logic - it can only correct things like a
          missing colon or unterminated string. This is the original,
          conservative behavior.

        - 'runtime': a genuine runtime/logic fix (off-by-one, wrong
          comparator, etc.) necessarily changes the AST, so AST-equality
          would reject every fix and make this mode useless. Instead, the
          fix is accepted only if it compiles AND the test suite passes
          against it. The test becomes the safety net in place of AST
          equality - i.e. we no longer guarantee "no logic changed", we
          guarantee "the fix is at least verified correct against the
          available tests" (ground-truth or LLM-generated, with the
          self-grading caveat noted in generate_tests()).
        """
...
```"""
            ...
            prompt = f"""The following Python code raises a runtime error. Fix the bug.
Keep the function signature the same. Return ONLY corrected code inside one ```python``` block.
...
```"""
        ...
        if error_class == "syntax":
            ...
            return fixed_code if run_candidate(fixed_code, test_code) == "PASS" else None
...
"""
    ...
    def run(self, prompt, test_code=None):
        ...
        if verdict.lower().startswith("fail"):
            ...
            if any(m in lowered for m in SYNTAX_ERROR_MARKERS):
                error_class = "syntax"
            elif any(m in lowered for m in RUNTIME_ERROR_MARKERS):
                error_class = "runtime"
            else:
                error_class = None  # unrecognized failure type, e.g. plain AssertionError mismatch -> no repair attempt

            if error_class is not None:
                fixed = self.debug(candidate_code, error_class, test_code)
